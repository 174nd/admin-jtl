...
import { QuestionGroupType } from './question/question.type';
import QuestionGroups from './question/QuestionGroups';
...
export default function ModalKepuasanKaryawan({openModal, closeModal, data}: {data: userData, openModal: boolean, closeModal: () => void}) {
  const totalNilai = data.questionGroups.reduce((r, v) => r + v.questions.reduce((r1, v1) => r1 + v1.answerKeys.reduce((r2, v2) => r2 < v2.point ? v2.point : r2, 0) ?? 0, 0), 0);
  const selectNilai = data.questionGroups.reduce((r, v) => r + v.questions.reduce((r1, v1) => r1 + v1.answerKeys.reduce((r2, v2) => v2.status ? v2.point : r2, 0) ?? 0, 0), 0);
  ...
  return (
    <Modal openModal={openModal} closeModal={closeModal}>
      <div className="w-full max-w-2xl bg-white shadow-md rounded-lg border">
        ...
        <QuestionGroups data={data.questionGroups} />
